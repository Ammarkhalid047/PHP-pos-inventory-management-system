<?php
	...
	mysqli_query($conn,"insert into purchase (userid, sales_total, sales_date) values ('".$_SESSION['id']."', '$new', NOW())");
	$sid=mysqli_insert_id($conn);
	
	$query=mysqli_query($conn,"select * from purchasecart where userid='".$_SESSION['id']."'");
	while($row=mysqli_fetch_array($query)){
		mysqli_query($conn,"insert into purchase_detail (purchaseid, productid, purchase_qty) values ('$sid', '".$row['productid']."', '".$row['qty']."')");
